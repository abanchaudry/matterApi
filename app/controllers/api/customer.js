...
router.post("/addNew",  async (req, res, next) => { 
  const customer = new Customer(req.body);
    ...
    //  let cstmr = await Customer.findOne({ customerEmail: req.body.customerEmail});
    //  if (cstmr) return res.json({ success: false, message: ' Email Already Exists' });
  customer.save().then(result => {
    if(result){
      res.status(200).json({success: true, message: "Customer Saved successful!" });
    }else{
      res.status(200).json({success: false, message: "Customer Not saved Please Try Again!" });
    }    
  });
});
